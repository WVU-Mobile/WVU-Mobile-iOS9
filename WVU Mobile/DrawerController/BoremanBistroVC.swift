...
class BoremanBistroVC: DiningHallVC {
    ...
    func alert() {
        let alertController = UIAlertController(title: "", message: "Get directions to Boreman Bistro?", preferredStyle: .Alert)
        ...
        let OKAction = UIAlertAction(title: "Directions", style: .Default) {
            (action) in
            if let url = NSURL(string: "http://maps.apple.com/?daddr=600+N+High+St+Morgantown+WV+26505+United+States,+CA&saddr=current+location") {
